...
export default function PlayerAuthPage() {
  ...
  const socialLogin = (platform: string) => {
    // Mock social login
    setError(`${platform} login va fi disponibil în curând!`);
  };
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 flex flex-col justify-between p-16 w-full">
          <div onClick={() => nav('/')} className="cursor-pointer">
            ...
            <p className="text-lime-500/60 font-bold uppercase tracking-widest text-[9px] mt-0.5">Basov Sports Hub</p>
          </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 lg:gap-4">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 lg:gap-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl lg:rounded-2xl py-3 lg:py-3.5 transition-all font-bold text-xs lg:text-sm"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 lg:gap-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl lg:rounded-2xl py-3 lg:py-3.5 transition-all font-bold text-xs lg:text-sm"
                  >
                    <Facebook className="w-4 h-4 lg:w-5 lg:h-5 fill-white text-white" />
                    Facebook
                  </button>
                ...
                {error && (
                   <div className="p-3 lg:p-4 rounded-xl lg:rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs lg:text-sm font-medium">
                     {error.includes("No static resource") ? (
                       "Eroare tehnică la server. Te rugăm să încerci logarea prin SMS/OTP."
                ...
                <form onSubmit={authMode === 'login' ? handleLogin : handleRegister} className="space-y-3 lg:space-y-4">
                   ...
                   <button 
                    ...
                    >
                      {loading ? 'Se încarcă...' : (authMode === 'login' ? 'Intră în Account' : 'Creează Cont')}
