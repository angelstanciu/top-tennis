...
function KeyButton({ icon, ro, en }: { icon: React.ReactNode; ro: string; en: string }) {
  return (
    <span className="mt-2.5 flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-2.5">
      <span className="shrink-0 w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-emerald-600">
        {icon}
      </span>
      <span className="flex flex-col min-w-0 leading-snug">
        <span className="font-extrabold text-slate-800 break-words" style={{ fontFamily: 'Outfit, sans-serif' }}>{ro}</span>
    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col">
      ...
      <div className="max-w-xl mx-auto w-full px-5 py-9 flex-1">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h2 className="mt-5 text-[1.75rem] leading-tight font-black text-slate-800" style={{ fontFamily: 'Outfit, sans-serif' }}>
            Instalează aplicația
          </h2>
          <p className="mt-2 text-slate-500 max-w-sm">
            Adaugă <strong className="text-slate-700">Star Arena</strong> pe ecranul principal și rezervi terenuri într-o secundă, ca dintr-o aplicație normală.
          </p>
        </div>

        {/* Card cu instrucțiuni în funcție de platformă */}
        <div className="bg-white rounded-[1.75rem] p-6 md:p-8 shadow-xl shadow-slate-200/60 border border-slate-100">
          ...
          {platform === 'installed' && (
            <div className="text-center py-4">
              ...
              <h3 className="mt-4 text-xl font-bold text-slate-800" style={{ fontFamily: 'Outfit, sans-serif' }}>Aplicația e deja instalată</h3>
              <p className="mt-2 text-slate-500">O găsești pe ecranul principal al telefonului. Poți rezerva direct de acolo.</p>
              <button
          ...
          {platform === 'android' && (
            <div>
              <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Outfit, sans-serif' }}>Instalare în 1 pas</h3>
              </div>
              {installPrompt ? (
                <>
                  <p className="text-slate-500 mb-5">Apasă butonul de mai jos, apoi confirmă în fereastra care apare.</p>
                  <button
                ...
                <>
                  <p className="text-slate-500 mb-5">Dacă nu apare butonul automat de instalare, adaug-o manual din meniul browserului — durează 10 secunde:</p>
                  <ol className="space-y-5">
                    <Step index={1}>
                      Apasă meniul cu trei puncte{' '}
                      <span className="inline-flex items-center justify-center w-6 h-6 rounded-md bg-slate-100 text-slate-700 align-middle mx-0.5"><DotsMenuIcon className="w-4 h-4" /></span>{' '}
          ...
          {platform === 'ios-safari' && (
            <div>
              <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Outfit, sans-serif' }}>3 pași simpli</h3>
              </div>
              <ol className="space-y-5">
                <Step index={1}>
                  În <strong className="text-slate-700">Safari</strong>, apasă butonul de partajare din bara de jos — e pătratul cu o săgeată în sus (pe iPad e sus, în dreapta):
              ...
              <div className="mt-6 flex items-start gap-2 text-xs text-slate-400 bg-slate-50 rounded-xl p-3">
          ...
          {platform === 'ios-other' && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Outfit, sans-serif' }}>Deschide în Safari</h3>
              </div>
              <p className="text-slate-500 mb-5">
                Pe iPhone, aplicația se poate instala <strong className="text-slate-700">doar din Safari</strong> (browserul Apple). Acum pagina e deschisă în alt browser. Copiază linkul, deschide Safari și lipește-l acolo:
              </p>
              <button
                onClick={handleCopy}
              ...
              <ol className="space-y-5 mt-7">
                <Step index={1}>Deschide aplicația <strong className="text-slate-700">Safari</strong>, lipește linkul în bara de adrese și apasă Enter.</Step>
                <Step index={2}>
          ...
          {platform === 'desktop' && (
            <div>
              <h3 className="text-lg font-bold text-slate-800 mb-3" style={{ fontFamily: 'Outfit, sans-serif' }}>
                Instalează pe telefon
              </h3>
              <p className="text-slate-500 mb-5">
                Aplicația e gândită pentru telefon. Deschide <strong className="text-slate-700">star-arena.ro/instalare</strong> pe mobil, sau trimite-ți linkul mai jos pe WhatsApp și instaleaz-o de pe telefon.
              </p>
              {installPrompt && (
                <button
                  onClick={handleInstall}
        ...
        {platform !== 'installed' && (
          <div className="mt-6 bg-white rounded-[1.75rem] p-5 md:p-6 shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide mb-1" style={{ fontFamily: 'Outfit, sans-serif' }}>
              Trimite mai departe
            </h3>
            <p className="text-sm text-slate-500 mb-4">Trimite cuiva linkul de instalare cu mesajul gata scris.</p>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button
                onClick={handleShare}
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-2xl transition-colors active:scale-[0.99] flex items-center justify-center gap-2"
              >
