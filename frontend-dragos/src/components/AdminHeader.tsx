...
export default function AdminHeader({ active }: AdminHeaderProps) {
  ...
  return (
    <div className="rounded-[2.5rem] overflow-hidden border border-slate-800 bg-slate-950 shadow-2xl mb-8">
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 px-6 py-5 border-b border-slate-800/50 flex flex-col gap-4">
        {/* Brand row: logo left, logout right — always on same line */}
        <div className="flex items-center justify-between">
          ...
          <button
             ...
             className="bg-slate-900 hover:bg-rose-500 text-slate-400 hover:text-white p-3.5 rounded-2xl border border-slate-800 transition-all active:scale-90 group shadow-lg shrink-0"
             title="Deconectare"
           >
        ...
        <div className="flex items-center bg-slate-950/50 p-1.5 rounded-2xl border border-slate-800 shadow-inner overflow-x-auto">
           {navItems.map(item => {
             ...
             return (
               <Link
                 key={item.id}
                 to={item.path}
                 className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 whitespace-nowrap ${isActive ? 'bg-emerald-500 text-white shadow-xl shadow-emerald-500/20 scale-105' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
               >
